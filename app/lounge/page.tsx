        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
            ...
            <div className="bg-surface p-1 rounded-xl border-2 border-slate-900 shadow-hard-sm flex self-start md:self-auto">
              <button 
                ...
                className={`px-6 py-2 rounded-lg text-xs font-black transition-all ${activeTab === 'solver' ? 'bg-slate-900 text-white shadow-sm' : 'text-slate-500 hover:text-slate-900'}`}
              >
                Solver (Jasa)
              </button>
              <button 
                onClick={() => setActiveTab('seeker')}
                className={`px-6 py-2 rounded-lg text-xs font-black transition-all ${activeTab === 'seeker' ? 'bg-slate-900 text-white shadow-sm' : 'text-slate-500 hover:text-slate-900'}`}
              >
                Seeker (Butuh)
              </button>
            </div>
          ...
          <div className="flex overflow-x-auto no-scrollbar gap-2 pb-1">
             {['#Jastip', '#TemanCurhat', '#JokiAntri', '#SurpriseUltah', '#Gaming', '#Absurd'].map((tag) => (
               <span key={tag} className="flex-shrink-0 px-3 py-1.5 bg-surface border-2 border-slate-200 text-slate-600 rounded-lg text-xs font-bold hover:border-slate-900 hover:text-slate-900 cursor-pointer transition select-none">
                 {tag}
               </span>
             ))}
          </div>
