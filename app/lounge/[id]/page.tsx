    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: gig.title,
          text: `Cek jasa unik ini di Jasurd: ${gig.title}`,
  ...
  return (
    <div className="min-h-screen bg-pattern pb-32 md:pb-12">
      ...
      <div className="fixed top-0 left-0 w-full z-40 px-4 py-4 pointer-events-none flex justify-between items-center">
        ...
        <button onClick={handleShare} className="pointer-events-auto inline-flex w-10 h-10 bg-white border-2 border-slate-900 rounded-full items-center justify-center text-slate-900 shadow-hard btn-brutal">
      ...
      <div className="md:hidden fixed bottom-0 left-0 w-full bg-white border-t-2 border-slate-900 p-4 z-50 flex items-center justify-between pb-8">
        ...
        <button onClick={() => setIsModalOpen(true)} className="bg-primary text-white px-8 py-3 rounded-xl font-black shadow-hard border-2 border-slate-900 text-sm active:scale-95 transition">
          Book Now
        </button>
      </div>
