...
export default function ArticleDetailPage() {
  ...
  const handleShare = async () => {
    if (navigator.share) {
      try { await navigator.share({ title: article.title, url: window.location.href }); } catch (e) {}
    } else {
  ...
  if (!article) return <div className="min-h-screen bg-white flex items-center justify-center font-bold">Artikel tidak ditemukan.</div>;
      ...
      <div className="h-64 md:h-96 relative w-full">
        ...
        <div className="absolute bottom-0 left-0 w-full p-6 md:p-10 bg-gradient-to-t from-black/90 to-transparent">
          <div className="max-w-3xl mx-auto">
            ...
            <p className="text-xs md:text-sm font-bold text-slate-300">{new Date(article.created_at).toLocaleDateString()} • Oleh {article.author}</p>
          </div>
        </div>
      </div>
